...
import java.util.*
open class GuiFluidStorage(_player: EntityPlayer, _guiName: String) : GuiContainer(
    ...
    override var currentFluid:IAEFluidStack? = null
        get() = containerFluidStorage.selectedFluidStack
    ...
    override fun drawGuiContainerForegroundLayer(mouseX: Int, mouseY: Int) {
        ...
        if (currentFluid != null) {
            ...
            if (shortenedBuckets) {
                when {
                    currentFluidAmount > 1000000000L -> amountToText = (currentFluidAmount / 1000000000L)
                            .toString() + "MegaB"
                    currentFluidAmount > 1000000L -> amountToText = (currentFluidAmount / 1000000L)
                            .toString() + "KiloB"
                    currentFluidAmount > 9999L -> {
    ...
    fun drawWidgets(mouseX: Int, mouseY: Int) {
        ...
        if (containerFluidStorage.fluidStackList?.isEmpty == false) {
            ...
            if (currentScroll > maxLine - 4) {
                currentScroll = (maxLine - 4).coerceAtLeast(0)
            }
        }
    }
    ...
    override fun initGui() {
        ...
        searchbar = object : GuiTextField(fontRendererObj,
            ...
            override fun mouseClicked(x: Int, y: Int, mouseBtn: Int) {
                val flag = x >= xPos1 && x < xPos1 + this.width1 && y >= yPos1 && y < yPos1 + this.height1
    ...
    fun updateFluids() {
        ...
        if (containerFluidStorage.fluidStackList != null) {
            for (fluidStack: IAEFluidStack? in containerFluidStorage.fluidStackList!!) {
                if (fluidStack != null && (fluidStack.fluid
    ...
    fun updateSelectedFluid() {
        currentFluid = null
        for (stack in containerFluidStorage.fluidStackList!!) {
            if (stack!!.fluid === containerFluidStorage.selectedFluid)
                currentFluid = stack
        }
    }
