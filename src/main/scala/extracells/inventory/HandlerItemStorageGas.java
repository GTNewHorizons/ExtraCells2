...
public class HandlerItemStorageGas implements IMEInventoryHandler<IAEFluidStack>, IHandlerFluidStorage {
    ...
    public HandlerItemStorageGas(ItemStack _storageStack, ISaveProvider _saveProvider, ArrayList<Fluid> _filter) {
        ...
        if (_filter != null) this.prioritizedFluids = _filter;
    }
    ...
    public boolean canAccept(IAEFluidStack input) {
        ...
        if (!ECApi.instance().isGasStack(input)) return false;
    ...
    public IAEFluidStack extractItems(IAEFluidStack request, Actionable mode, BaseActionSource src) {
        if (request == null || !allowedByFormat(request.getFluid())) return null;
        ...
        for (int i = 0; i < this.fluidStacks.size(); i++) {
            ...
            if (currentStack != null && currentStack.getFluidID() == request.getFluid().getID()) {
                ...
                if (removedStack != null && removedStack.getStackSize() > 0) requestSave();
                return removedStack;
            }
        }
    ...
    public int freeBytes() {
        int i = 0;
        for (FluidStack stack : this.fluidStacks) if (stack != null) i += stack.amount;
    ...
    public IAEFluidStack injectItems(IAEFluidStack input, Actionable mode, BaseActionSource src) {
        if (input == null || !allowedByFormat(input.getFluid())) return input;
        ...
        if (notAdded == null || !notAdded.equals(input)) requestSave();
        return notAdded;
    }
    ...
    private void requestSave() {
        if (this.saveProvider != null) this.saveProvider.saveChanges(this);
    }
    ...
    public int usedTypes() {
        int i = 0;
        for (FluidStack stack : this.fluidStacks) if (stack != null) i++;
        return i;
    }
